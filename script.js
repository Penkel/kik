...
document.querySelector('.glow').addEventListener("click", () => {document.querySelector('.glow').classList.remove('glow')}, {once : true})

const volumeSlider = document.querySelector('.volume-slider');
const audio = document.querySelector('audio');
volumeSlider.addEventListener('input', () => {audio.volume = volumeSlider.value * 0.01});
...
heightSlider.addEventListener('input', () => {artCont.style.height = heightSlider.value + 'vh'})

const epicArtButton = document.querySelector('.epicArtButton');
let epicShit
function toggleEpicArt() { 
    document.querySelectorAll('.epic').forEach(epic => {epic.style.removeProperty('filter'); console.log(epic)})
    fullScreen.style.removeProperty('filter');
    document.querySelectorAll('.epic').forEach(epic => {clearInterval(epicShit)})
    ...
    epicShit = setInterval(() => {
        ...
        epic.style.filter = `hue-rotate(${epicMeter.power * 99}deg) contrast(${epicMeter.power}) blur(${epicMeter.power / 3}px) saturate(${Math.random() *6})`;
    }, 10)}) 
} else {
...
async function getLocs() {
    try {
        await fetch('./slides.json').then(results => results.json()).then(data => showInfo(data));
        ...
        await locs.forEach(element => {
            ...
            advLink.addEventListener('click', () => {changeAdventure(locs.indexOf(element))});
        });
...
function changeAdventure(indx) {
    ...
    document.querySelectorAll('.adv-select-button')[advIndex].classList.remove('selected');
    document.querySelectorAll('.adv-select-button')[indx].classList.add('selected');
    advIndex = indx;
    console.log(advIndex);
    root.style.setProperty('--cur-art', "url(./arts/" + locs[indx].locations[0].pic + ")")
    ...
    if (!muteCheckbox.checked){audio.play()}
...
function changeLoc() {
    ...
    let currentLoc = curLocs.find(({code}) => code === userInput)
